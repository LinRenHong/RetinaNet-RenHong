...
def main(args=None):
	...
	parser.add_argument('--load_model_path', help='Path to model (.pt) file.')
	...
	if parser.dataset_type == 'coco':
		...
		# dataset_val = CocoDataset(parser.coco_path, set_name='train', transform=transforms.Compose([Normalizer(), Resizer()]))
		# dataset_val = CocoDataset(parser.coco_path, set_name='val', transform=transforms.Compose([Normalizer(), Resizer()]))
		dataset_val = CocoDataset(parser.coco_path, set_name='test', transform=transforms.Compose([Normalizer(), Resizer()]))
	...
	for idx, data in enumerate(dataloader_val):
		...
		with torch.no_grad():
			...
			#cv2.imshow('img', img)
			#cv2.waitKey(0)
			cv2.imwrite('D:\\StreetView\\RenHong\\Pytorch_RetinaNet\\results\\images\\'+str(idx)+'.jpg', img)
