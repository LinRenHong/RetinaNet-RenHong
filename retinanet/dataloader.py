...
### add data augmentation by RenHong ###
import imgaug as ia
...
class AugmenterWithImgaug(object):
    ...
    def __call__(self, sample):
        ...
        bbs_on_image_list = []
        for annot in annots:
            bbs_on_image_list.append(BoundingBox(x1=annot[0], y1=annot[1], x2=annot[2], y2=annot[3], label=annot[4]))

        bbs = BoundingBoxesOnImage(bbs_on_image_list, shape=image.shape)
        ...
        image_aug, bbs_aug = seqAug(image=image, bounding_boxes=bbs)
        ...
        for i in range(len(bbs.bounding_boxes)):
            after = bbs_aug.bounding_boxes[i]
            annot_aug_list.append([after.x1, after.y1, after.x2, after.y2, after.label])

        annots_aug = np.array(annot_aug_list, dtype=float)

        # # image with BBs before/after augmentation (shown below)
        # image_before = bbs.draw_on_image(image, size=2)
        # image_after = bbs_aug.draw_on_image(image_aug, size=2, color=[0, 0, 255])

        # cv2.imwrite('t1_1.jpg', (image_before * 255).astype('uint8'))
        # cv2.imwrite('t1_2.jpg', (image_after * 255).astype('uint8'))
